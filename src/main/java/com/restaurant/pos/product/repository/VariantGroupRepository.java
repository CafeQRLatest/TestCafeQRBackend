...
public interface VariantGroupRepository extends JpaRepository<VariantGroup, UUID> {
    ...
    @Query("SELECT DISTINCT v FROM VariantGroup v WHERE (v.clientId = :clientId OR v.clientId IS NULL) AND (v.orgId = :orgId OR v.orgId IS NULL)")
    List<VariantGroup> findByClientIdAndOrgIdOrGlobal(UUID clientId, UUID orgId);

    @EntityGraph(attributePaths = {"options"})
    @Query("SELECT DISTINCT v FROM VariantGroup v WHERE (v.clientId = :clientId OR v.clientId IS NULL) AND (v.orgId = :orgId OR v.orgId IS NULL) AND v.isActive = true")
    List<VariantGroup> findByClientIdAndOrgIdOrGlobalAndIsActiveTrue(UUID clientId, UUID orgId);

    @EntityGraph(attributePaths = {"options"})
    @Query("SELECT DISTINCT v FROM VariantGroup v WHERE (v.clientId = :clientId OR v.clientId IS NULL) AND (v.orgId = :orgId OR v.orgId IS NULL) AND v.updatedAt >= :updatedAfter")
